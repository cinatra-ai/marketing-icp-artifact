...
For development, clone this repository and run `node extension-kind-gate.mjs --package-root .` to validate the extension manifest and README before publishing. The authoring skill lives in `@cinatra-ai/marketing-icp-authoring-skill` and the matcher in `@cinatra-ai/marketing-icp-matcher-skill`. If classification misses a document, check whether the document's dominant content is a different artifact type — strategy, brand voice, or sales playbook documents are intentionally excluded.
